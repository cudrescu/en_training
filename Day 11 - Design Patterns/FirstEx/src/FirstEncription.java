...
public class FirstEncription implements Encription{

	private ArrayList<LetterMapping> letterMappingArray;
	private HashMap<String, Integer> letterMappingHash;
	
	FirstEncription(ArrayList<LetterMapping> letterMappingArray, HashMap<String, Integer> letterMappingHash){
			...
			case 0:
				for(int i = 0; i<length; i++){
					if((toCriptArray[i] >= 'a' && toCriptArray[i] <= 'z') || (toCriptArray[i] >= 'A' && toCriptArray[i] <= 'Z')){
							cripted += getMappedValue(toCriptArray[i]);
					}
				}
				break;
			case 1:
				for(int i = 0; i<length; i++){
					if((toCriptArray[i] >= 'a' && toCriptArray[i] <= 'z') || (toCriptArray[i] >= 'A' && toCriptArray[i] <= 'Z')){
							cripted += letterMappingHash.get(String.valueOf(toCriptArray[i]));
					}
				}
				break;
			default:
		...
		switch(key){
			...
			case 0:
				for(int i = 0; i<length; i+=2){
					decripted += getMappedLetter(Integer.parseInt(""+toDecriptArray[i] + toDecriptArray[i+1]), 0);
				}
				break;
			case 1:
				for(int i = 0; i<length; i+=2){
					decripted += getMappedLetter(Integer.parseInt(""+toDecriptArray[i] + toDecriptArray[i+1]), 1);
				}
				break;
			default:
				System.out.println("Wrong type argument! Expected 0/1");
