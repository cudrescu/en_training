...
public class Main{
	...
	public static void main(String[] args){
		...
		int chunkSize = testSize/noThreads;
		ComputeThread computeThreads[] = new ComputeThread[noThreads];
		
		for(int i = 0; i<noThreads; i++){
			if(i != noThreads - 1){
				computeThreads[i] = new ComputeThread(initialList, resultList, i*chunkSize, (i+1)*chunkSize);
			}
			else{
				computeThreads[i] = new ComputeThread(initialList, resultList, i*chunkSize, testSize);
			}
